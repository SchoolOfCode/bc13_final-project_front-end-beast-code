    ...
    <div className={styles.whole_page}>
      ...
      <div className={styles.about_us}>
        ...
        <div className={styles.all_profiles}>
          {team.map((each) => (<AboutUsProfile key={each.fullname} profileImage={each.profileImage} name={each.fullname} tenSeconds={each.tenSeconds} linkedIn={each.linkedIn} gitHub={each.gitHub}/>))}
        </div>
        <h1>The Project in Numbers</h1>
        <hr className={styles.lines}></hr>
        <p>some fas number magic</p>
        <h1>What we built & Why</h1>
        <hr className={styles.lines}></hr>
        <h3 className={styles.project_description}>For the final 4-week project of the SoC bootcamp our team took a problem-first approach to ideation:</h3>
        <p className={styles.project_description}>We decided to create an app that solves the problem of needing to visit several websites and do a frustratingly large number of google searches to plan a night out - and still not finding all the information required to be sure a bar is right for you!</p>
        <p className={styles.project_description}>Using different filters and search capabilities including a map feature, Cheers users have all the information they could possibly desire at their fingertips - no matter the group, occasion or features required.</p>
